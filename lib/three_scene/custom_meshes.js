import { MeshStandardMaterial, Mesh, CapsuleGeometry, BoxGeometry } from 'three'
...
export function createBuildingMesh(building) {
  ...
  if (building instanceof Barrack) return createBarracksMesh()
...
export function createCitizenMesh(citizen) {
  if (citizen instanceof Chief) return createCapsuleMesh(0.5, 0.25, '#ffffff')
  if (citizen instanceof Doctor) return createCapsuleMesh(0.5, 0.25, '#00ff00')
  if (citizen instanceof Worker) return createCapsuleMesh(0.5, 0.25, '#0000ff')
  if (citizen instanceof Trainer) return createCapsuleMesh(0.5, 0.25, '#ff0000')
  if (citizen instanceof Melee) return createCapsuleMesh(0.5, 0.25, '#ffff00')
  if (citizen instanceof Ranged) return createCapsuleMesh(0.5, 0.25, '#00ffff')
  return createBoxMesh(1, 2, 1, '#ff00ff')
}
